...
/// Matches the encodings [`to_arrow_list`] converts directly: `List` (zero-copy offsets) and
/// `Chunked` (per-chunk conversion + Arrow concat).
struct ListExportable;

impl Matcher for ListExportable {
    type Match<'a> = &'a ArrayRef;

    fn matches(array: &ArrayRef) -> bool {
        array.is::<List>() || array.is::<Chunked>()
    }
...
mod tests {
    ...
    #[test]
    fn slice_wrapped_list_exports_via_list_fast_path() -> VortexResult<()> {
        let mut ctx = SESSION.create_execution_ctx();
        ...
        let offsets_ptr = offsets.as_slice::<i32>().as_ptr();
        ...
        // The Slice execute-parent kernel reveals the List encoding, whose conversion reuses
        // both the elements buffer and the (sliced) offsets buffer zero-copy; the ListView
        // fallback would materialize a new offsets buffer.
        ...
        assert_eq!(arrow_list.offsets().as_ptr(), offsets_ptr.wrapping_add(1));
        Ok(())
    }

    #[test]
    fn mask_wrapped_list_exports_via_list_fast_path() -> VortexResult<()> {
        let mut ctx = SESSION.create_execution_ctx();
        ...
        // The mask execute-parent kernel reveals the List encoding (validity only), so the
        // conversion still shares the elements buffer.
        let values = arrow_list
            .values()
            .as_any()
            .downcast_ref::<Int32Array>()
