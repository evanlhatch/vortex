...
/// Matches the encodings [`to_arrow_run_end`] converts directly — `RunEnd` and `Constant` —
/// avoiding a decode-then-re-detect-runs round trip through `arrow_cast`.
struct RunEndExportable;

impl Matcher for RunEndExportable {
    type Match<'a> = &'a ArrayRef;

    fn matches(array: &ArrayRef) -> bool {
        array.is::<RunEnd>() || array.is::<Constant>()
    }
...
pub(super) fn to_arrow_run_end(
    ...
    let array = reveal_for_export::<RunEndExportable>(array, ctx)?;
