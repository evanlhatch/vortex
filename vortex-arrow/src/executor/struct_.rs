...
/// Matches the shapes [`to_arrow_struct`] short-circuits on: `Struct`, `Chunked`
/// (chunk-of-struct swizzled to struct-of-chunk), and the `pack` scalar function.
struct StructExportable;

impl Matcher for StructExportable {
...
pub(super) fn to_arrow_struct(
    ...
    let array = reveal_for_export::<StructExportable>(array, ctx)?;
