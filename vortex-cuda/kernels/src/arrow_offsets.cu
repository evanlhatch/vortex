...
#define GENERATE_FROM_LENGTHS_KERNEL(suffix, LengthT, is_signed)                                            \
    extern "C" __global__ void arrow_offsets_from_lengths_##suffix(                                        \
        const LengthT *__restrict lengths, int32_t *__restrict scan, uint32_t *status, uint64_t len) {      \
...
extern "C" __global__ void arrow_offsets_validate(const int32_t *__restrict offsets,
                                                   uint32_t *status,
                                                   uint64_t scan_len) {
    const uint64_t elements_per_block = (uint64_t)blockDim.x * ELEMENTS_PER_THREAD;
    const uint64_t block_start = (uint64_t)blockIdx.x * elements_per_block;
