...
pub(super) fn initialize(session: &VortexSession) {
    ...
    kernels.register_execute_parent_kernel(Slice.id(), RunEndBool, RunEndBoolSliceKernel);
    kernels.register_execute_parent_kernel(
        Filter.id(),
        RunEndBool,
        FilterExecuteAdaptor(RunEndBool),
    );
    kernels.register_execute_parent_kernel(Dict.id(), RunEndBool, TakeExecuteAdaptor(RunEndBool));
    kernels.register_execute_parent_kernel(Not.id(), RunEndBool, NotExecuteAdaptor(RunEndBool));
}
