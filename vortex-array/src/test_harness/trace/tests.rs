...
fn trace_take_on_chunked() -> VortexResult<()> {
    ...
    execute_until target=AnyCanonical root=vortex.primitive(i32, len=2)
      ...
      return output=vortex.primitive(i32, len=3)
        child_execute_parent static:kernel[3] slot=1 parent=vortex.dict(i32, len=4) child=vortex.chunked(i32, len=5) -> vortex.dict(i32, len=4)
...
fn trace_filter_on_struct_with_complex_children() -> VortexResult<()> {
    ...
    .into_array();
    assert_arrays_eq!(traced.output, expected);
    insta::assert_snapshot!(traced.trace.to_string(), @"
...
fn trace_compare_on_dict() -> VortexResult<()> {
    ...
    execute_until target=AnyCanonical root=vortex.constant(i32?, len=1)
      ...
      iter 3 current=vortex.dict(bool, len=5) builder_active=false
        child_execute_parent static:kernel[2] slot=1 parent=vortex.dict(bool, len=5) child=vortex.bool(bool, len=3) -> vortex.bool(bool, len=5)
...
fn trace_like_on_dict() -> VortexResult<()> {
    ...
    optimize root=vortex.cast(utf8?, len=3) session=false
      reduce_parent static:CastReduceAdaptor(VarBinView) slot=0 parent=vortex.cast(utf8?, len=3) child=vortex.varbinview(utf8, len=3) -> vortex.varbinview(utf8?, len=3)
      done output=vortex.varbinview(utf8?, len=3)
    optimize root=vortex.slice(utf8, len=1) session=false
      reduce_parent static:SliceReduceAdaptor(Constant) slot=0 parent=vortex.slice(utf8, len=1) child=vortex.constant(utf8, len=3) -> vortex.constant(utf8, len=1)
      done output=vortex.constant(utf8, len=1)
    optimize root=vortex.cast(utf8?, len=1) session=false
      reduce_parent static:CastReduceAdaptor(Constant) slot=0 parent=vortex.cast(utf8?, len=1) child=vortex.constant(utf8, len=1) -> vortex.constant(utf8?, len=1)
      done output=vortex.constant(utf8?, len=1)
    execute_until target=AnyCanonical root=vortex.constant(utf8?, len=1)
      iter 0 current=vortex.constant(utf8?, len=1) builder_active=false
        Done array=vortex.varbinview(utf8?, len=1)
      iter 1 current=vortex.varbinview(utf8?, len=1) builder_active=false
      return output=vortex.varbinview(utf8?, len=1)
        Done array=vortex.bool(bool, len=3)
      ...
      iter 3 current=vortex.dict(bool, len=6) builder_active=false
        child_execute_parent static:kernel[2] slot=1 parent=vortex.dict(bool, len=6) child=vortex.bool(bool, len=3) -> vortex.bool(bool, len=6)
